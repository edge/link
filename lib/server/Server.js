...
export class Server {
  ...
  onConnection(socket) {
    ...
    this._events.emit('connection', socket)
  }
  ...
  async onPreAuthMessage(ws, data) {
    let msg
    try {
      msg = JSON.parse(data)
    } catch (err) {
    ...
    if (this.onAuthenticate) {
      const result = await this.onAuthenticate(msg.address)
      log('Custom authentication result:', result)
      if (!result.success) {
      ...
      // If data is returned, store it and attach to client later
      if (result.data) authenticationData = result.data
    }
    ...
    if (authenticationData) client.authenticationData = authenticationData
    this._pendingAuth.delete(ws)
